dea.fast <- function(base, noutput, fixed = NULL, rts = 2, bound = NULL,
                     add.model = c(
                       "additive", "RAM", "BAM", "MIP",
                       "LovPast", "SBM"
                     ), blockSize = 200) {
  .Defunct(
    package = "additiveDEA",
    msg = "'dea.fast' is defunct. The speed of the DEA functions has been significantly improved- dea.fast() is no longer required."
  )
  ...
  for (i in 1:mod) {
    ...
    if (add.model != "SBM") {
      eff <- round(dea.gem(
        base = base1, noutput, fixed, rts,
        bound = bound1, add.model
      )$eff, 7)
      index <- which(is.na(eff))
      if (length(index) > 0) {
        eff[index] <- round(dea.gem(
          base = base1, noutput, fixed, rts,
          bound = bound1, add.model, whichDMUs = index
        )$eff, 7)
      }
      ...
      index <- which(is.na(eff))
      if (length(index) > 0) {
        eff[index] <- round(dea.sbm(
          base = base1, noutput, fixed, rts,
          bound = bound1, whichDMUs = index
        )$eff, 7)
      }
  ...
  for (i in 1:mod) {
    ...
    if (add.model != "SBM") {
      eff[[i]] <- dea.gem(
        base = base1, noutput, fixed, rts,
        bound = bound1, add.model, whichDMUs = 1:blockSize
      )$eff
      ...
      eff[[i]] <- dea.sbm(
        base = base1, noutput, fixed, rts,
        bound = bound1, whichDMUs = 1:blockSize
      )$eff
      index <- which(is.na(eff[[i]]))
      if (length(index) > 0) {
        eff[[i]][index] <- dea.sbm(
  ...
  if (n %% blockSize != 0) {
    ...
    if (add.model != "SBM") {
      eff[[i + 1]] <- dea.gem(
        base = base1, noutput, fixed, rts,
        bound = bound1, add.model, whichDMUs = 1:newBlockSize
      )$eff
      index <- which(is.na(eff[[i + 1]]))
      if (length(index) > 0) {
        eff[[i + 1]][index] <- dea.gem(
          base = base1, noutput, fixed, rts,
          bound = bound1, add.model, whichDMUs = index
        )$eff
      }
      ...
      eff[[i + 1]] <- dea.sbm(
        base = base1, noutput, fixed, rts,
        bound = bound1, whichDMUs = 1:newBlockSize
      )$eff
      index <- which(is.na(eff[[i + 1]]))
      if (length(index) > 0) {
        eff[[i + 1]][index] <- dea.sbm(
          base = base1, noutput, fixed, rts,
          bound = bound1, whichDMUs = index
        )$eff
      }
